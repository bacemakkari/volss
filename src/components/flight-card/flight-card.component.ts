  ...
  template: `
    <div class="flight-card bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
      <div class="flex items-start justify-between">
        <!-- Flight Details -->
        <div class="flex-1">
          <div class="text-sm text-gray-600 font-medium mb-3">{{ flight.compagnie }}</div>
          
          <div class="flex items-center space-x-6">
            <!-- Departure -->
            <div class="text-center">
              <div class="text-lg font-bold text-gray-900">{{ flight.heure_depart }}</div>
              <div class="text-xs text-gray-600 font-medium">{{ getAirportCode(flight.ville_depart) }}</div>
            </div>
            ...
            <div class="flex-1 flex flex-col items-center">
              ...
              <div class="text-xs text-green-600 mt-1 font-medium bg-green-100 px-2 py-1 rounded">{{ flight.direct ? 'Direct' : flight.escales + ' escale' + (flight.escales > 1 ? 's' : '') }}</div>
            </div>

            <!-- Arrival -->
            <div class="text-center">
              <div class="text-lg font-bold text-gray-900">{{ flight.heure_arrivee }}</div>
              <div class="text-xs text-gray-600 font-medium">{{ getAirportCode(flight.ville_arrivee) }}</div>
            </div>
          </div>
        ...
        <div class="ml-6 text-right flex flex-col items-end">
          <div class="flex items-center mb-2">
            <div class="text-xs text-gray-500 mr-2">{{ flight.offres || '13' }} offres dès</div>
